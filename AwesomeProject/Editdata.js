...
const Createdata = () => {
    const jsonUrl = 'http://192.168.201.99:3000/mahasiswa'; //API yang digunakan emulator untuk mengakses lokalhost desktop
    ...
    fetch(`http://192.168.201.99:3000/mahasiswa/${selectedUser.id}`, {
        method: 'PATCH',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
            },
            body: JSON.stringify(data) //Mengubah data menjadi bentuk json
        })
            .then((response) => response.json())
            .then((json) => {
                console.log(json);
